from MPVisitor import MPVisitor
from MPParser import MPParser
from AST import *
import functools
class ASTGeneration(MPVisitor):
    def toBool(self,x):
        return x == "True"

    # def visitProgram(self,ctx:MPParser.ProgramContext):
        # return self.visit(ctx.exp())

    # def visitExp(self,ctx:MPParser.ExpContext):
        # return Binary(ctx.ASSIGN().getText(),self.visit(ctx.term()),self.visit(ctx.exp())) if ctx.ASSIGN() else self.visit(ctx.term())

    # def visitTerm(self,ctx:MPParser.TermContext): 
        # return Binary(ctx.COMPARE().getText(),self.visit(ctx.factor(0)),self.visit(ctx.factor(1))) if ctx.COMPARE() else self.visit(ctx.factor(0))

    # def visitFactor(self,ctx:MPParser.FactorContext):
        # return Binary(ctx.ANDOR().getText(),self.visit(ctx.factor()),self.visit(ctx.operand())) if ctx.ANDOR() else self.visit(ctx.operand())
  
    # def visitOperand(self,ctx:MPParser.OperandContext):
        # if ctx.exp():
            # return self.visit(ctx.exp())
        # elif ctx.ID():
            # return Id(ctx.ID().getText())
        # elif ctx.INTLIT():
            # return IntLiteral(int(ctx.INTLIT().getText()))
        # else:
            # return BooleanLiteral(bool(ctx.BOOLIT().getText()))
        
    def visitProgram(self,ctx:MPParser.ProgramContext):
        return self.visit(ctx.exp())

    def visitExp(self,ctx:MPParser.ExpContext):
        lst = list(zip(ctx.term()[:-1],ctx.ASSIGN()))[::-1]
        return functools.reduce(lambda x,y: Binary(y[1].getText(),self.visit(y[0]),x),lst,self.visit(ctx.term()[-1]))

    def visitTerm(self,ctx:MPParser.TermContext): 
        return Binary(ctx.COMPARE().getText(),self.visit(ctx.factor(0)),self.visit(ctx.factor(1))) if ctx.COMPARE() else self.visit(ctx.factor(0))

    def visitFactor(self,ctx:MPParser.FactorContext):
        lst = list(zip(ctx.ANDOR(),ctx.operand()[1:]))
        return functools.reduce(lambda x,y: Binary(y[0].getText(),x,self.visit(y[1])),lst,self.visit(ctx.operand()[0]))

    def visitOperand(self,ctx:MPParser.OperandContext):
        if ctx.exp():
            return self.visit(ctx.exp())
        elif ctx.ID():
            return Id(ctx.ID().getText())
        elif ctx.INTLIT():
            return IntLiteral(int(ctx.INTLIT().getText()))
        else:
            #return BooleanLiteral(bool(ctx.BOOLIT().getText()))
            return BooleanLiteral(self.toBool(ctx.BOOLIT().getText()))